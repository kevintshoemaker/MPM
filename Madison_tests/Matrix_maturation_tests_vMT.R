...
mat <- do_fas(scen1$surv,scen1$fec,scen1$dur)  # FAS
mat
...
# stab = data.frame(stage_name=paste0("s",c(1:nstag)),survival=c(NULL,surv),maternity=c(NULL,0,fec),duration=c(NULL,ma,Inf))
# mat2 <- make_stage4age_matrix(stab,approx_method = "AAS",model="pre")
# mat2[1,2]  = fec
# mat2          # why does kendall's function increase the adult survival rate?
# mat2[2,2] = surv[2]
# lambda(mat2)   # not totally analogous- not sure what Kendall's function is doing exactly.. 

# check result against kendall's package
# mat2 <- make_stage4age_matrix(stab,approx_method = "unrolled",model="pre")
# mat2[1,9]  = fec
