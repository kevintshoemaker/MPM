...
# Helper Functions for matrix population modeling in R ----------
...
#  s is vector of survival, f is fecundity (scalar)
...
### Function to compute the objective of max entropy (minimize the negative entropy) for discrete distribution
     # p is a vector of probabilities that sum to 1, and the entropy is computed as the expected value of log p
negent = function(p){
  sum(p*log(p))
}
...
# sm is mean survival rate
# sa is starting survival rate
# sz is ending survival rate
# y is number of years to "ramp up"

# sm = 0.75; sa = 0.5; sz = 0.95; y= 5; include_first=T; include_last=F    # note: t should be integer
...
mean(ramp_fun(0.75,0.4,0.9,6,include_first = T,include_last = F))
